...
class FieldMappingService:
	...
	def map_fields_dynamically(
		self, parsed_data, field_mapping, policy_record, policy_type, protected_fields=None
	):
		"""
			...
			if policy_field_name:
				...
				if protected_fields and policy_field_name in protected_fields:
					...
					if current_value:
						protected_count += 1
						frappe.logger().info(
							f"⊘ Skipping {policy_field_name} - already set from Policy Document (value: {current_value})"
						)
						continue
	...
	def _convert_to_select(self, value, options, field_name=None):
		...
		if default_value:
			frappe.logger().warning(
				f"Select field '{field_name}' value '{value_str}' not found in options. Using default: '{default_value}'"
			)
			return default_value

		# If no default defined, return None to leave field empty
		frappe.logger().warning(
			f"Select field '{field_name}' value '{value_str}' not found in options. Leaving empty."
		)
		return None
