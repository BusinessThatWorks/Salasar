...
class PolicyCreationService:
	...
	def create_policy_record(self, policy_document_name, policy_type):
		...
		CommonService.validate_required_fields(
			{"policy_document_name": policy_document_name}, ["policy_document_name"]
		)
		if not isinstance(policy_document_name, str):
			frappe.throw(
				f"Invalid input: policy document name must be a string, got {type(policy_document_name).__name__}: {policy_document_name}"
			)
		...
		try:
			...
			frappe.logger().info(
				f"Field mapping retrieved: {len(field_mapping) if field_mapping else 0} entries"
			)

			if not field_mapping:
				frappe.logger().error(f"No field mapping found for {policy_type}")
				frappe.throw(
					f"No field mapping found for {policy_type}. Please refresh field mappings in Policy Reader Settings."
				)
			...
			frappe.logger().info(
				f"Parsed data keys: {list(parsed_data.keys()) if parsed_data else 'No parsed data'}"
			)
			frappe.logger().info(
				f"Parsed data sample: {dict(list(parsed_data.items())[:5]) if parsed_data else 'No data'}"
			)
	...
	def _populate_customer_fields(self, policy_record, policy_doc):
		...
		try:
			if not policy_doc.customer_code:
				frappe.logger().info(
					"No customer selected in Policy Document, skipping customer field population"
				)
				return
			...
			frappe.logger().info(
				f"Successfully copied customer fields from Policy Document for {policy_record.doctype}"
			)
		...
		try:
			...
			for source_field, target_field in checklist_mapping.items():
				...
				if value:
					...
					copied_count += 1
					frappe.logger().info(
						f"Copied checklist field: {source_field} -> {target_field} = {value}"
					)
	...
	def _copy_business_info_fields(self, policy_record, policy_doc):
		...
		try:
			if not policy_doc.insurance_company_branch:
				frappe.logger().info(
					"No insurance company branch selected in Policy Document, skipping business info population"
				)
				return
			...
			frappe.logger().info(
				f"Successfully copied business info fields from Policy Document for {policy_record.doctype}"
			)
	...
	def map_fields_dynamically(self, parsed_data, field_mapping, policy_record, policy_type):
		...
		for raw_key, raw_value in parsed_data.items():
			...
			if policy_field_name:
				try:
					converted_value = self.convert_field_value(
						policy_field_name, raw_value, policy_record.doctype
					)
		...
		return {"mapped_count": mapped_count, "unmapped_fields": unmapped_fields, "suggestions": suggestions}
	...
	def convert_field_value(self, field_name, value, doctype):
		...
		try:
			...
			if field.fieldtype == "Date":
				...
				return self.normalize_select_value(
					value, field.options, field_name=field_name, field_label=field.label
				)
			else:
				return cstr(value)
	...
	def normalize_select_value(self, value, options, field_name=None, field_label=None):
		...
		if default_value:
			frappe.logger().warning(
				f"Select field '{field_name or field_label}' value '{value}' not found in options. Using default: '{default_value}'"
			)
			return default_value

		# If no default defined, return None to leave field empty
		frappe.logger().warning(
			f"Select field '{field_name or field_label}' value '{value}' not found in options. Leaving empty."
		)
		return None
	...
	def validate_policy_creation_prerequisites(self, policy_document_name):
		...
		try:
			...
			if not policy_doc.extracted_fields:
				return {"valid": False, "error": "No extracted fields found. Please extract fields first."}

			# Check if policy type is set
			if not policy_doc.policy_type:
				return {"valid": False, "error": "Policy type is not set. Please set the policy type first."}

			# Check if policy already exists
			if policy_doc.policy_type.lower() == "motor" and policy_doc.motor_policy:
				return {"valid": False, "error": "Motor policy already exists for this document."}
			elif policy_doc.policy_type.lower() == "health" and policy_doc.health_policy:
				return {"valid": False, "error": "Health policy already exists for this document."}
