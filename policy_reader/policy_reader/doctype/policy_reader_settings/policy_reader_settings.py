...
class PolicyReaderSettings(Document):
	...
	def test_api_connection(self):
		...
		try:
			...
			frappe.msgprint(
				"API key format appears valid. Test connection functionality can be implemented as needed.",
				title="API Key Test",
				indicator="green",
			)
	...
	def test_saiba_connection(self):
		...
		if not self.saiba_base_url or not self.saiba_username or not self.saiba_password:
			return {
				"success": False,
				"error": "SAIBA credentials are not fully configured. Please fill in Base URL, Username, and Password.",
			}

		try:
			# Import locally to avoid circular imports
			from policy_reader.policy_reader.services.saiba_sync_service import (
				test_saiba_connection as _test_saiba_connection,
			)
	...
	def refresh_field_mappings(self):
		...
		try:
			...
			frappe.msgprint(
				...
				title="Field Mappings Refreshed",
				indicator="green",
			)

			return {"success": True, "motor_fields": len(motor_mapping), "health_fields": len(health_mapping)}

		except Exception as e:
			frappe.log_error(
				f"Unexpected error while refreshing field mappings: {str(e)}", frappe.get_traceback()
			)
		...
		if policy_type_lower == "motor":
			alias_map = {
				...
				"cc": [
					...
					"CC/KW",
					"Cubic Capcity",
					"Cubic Capacity/Kilowatt",
					"Cubic Capcity/Kilowatt",
					"CCIKW",
				],
			...
			alias_map = {
				...
				"insured_1_sum_insured": [
					"INSURED1SUMINSURED",
					"Insured1SumInsured",
					"Insured 1 Sum Insured",
					"insured_1_sum_insured",
					"insured1suminsured",
				],
				# Insured Person 2
				...
				"insured_2_sum_insured": [
					"INSURED2SUMINSURED",
					"Insured2SumInsured",
					"Insured 2 Sum Insured",
					"insured_2_sum_insured",
					"insured2suminsured",
				],
				# Insured Person 3
				...
				"insured_3_sum_insured": [
					"INSURED3SUMINSURED",
					"Insured3SumInsured",
					"Insured 3 Sum Insured",
					"insured_3_sum_insured",
					"insured3suminsured",
				],
				# Insured Person 4
				...
				"insured_4_sum_insured": [
					"INSURED4SUMINSURED",
					"Insured4SumInsured",
					"Insured 4 Sum Insured",
					"insured_4_sum_insured",
					"insured4suminsured",
				],
				# Insured Person 5
				...
				"insured_5_sum_insured": [
					"INSURED5SUMINSURED",
					"Insured5SumInsured",
					"Insured 5 Sum Insured",
					"insured_5_sum_insured",
					"insured5suminsured",
				],
				# Insured Person 6
				...
				"insured_6_sum_insured": [
					"INSURED6SUMINSURED",
					"Insured6SumInsured",
					"Insured 6 Sum Insured",
					"insured_6_sum_insured",
					"insured6suminsured",
				],
				# Insured Person 7
				...
				"insured_7_sum_insured": [
					"INSURED7SUMINSURED",
					"Insured7SumInsured",
					"Insured 7 Sum Insured",
					"insured_7_sum_insured",
					"insured7suminsured",
				],
				# Insured Person 8
				...
				"insured_8_sum_insured": [
					"INSURED8SUMINSURED",
					"Insured8SumInsured",
					"Insured 8 Sum Insured",
					"insured_8_sum_insured",
					"insured8suminsured",
				],
				# Financial Details
	...
	def build_field_mapping_from_doctype(self, doctype_name):
		...
		try:
			...
			frappe.log_error(
				f"Error building default mapping for {doctype_name}: {str(e)}", "Field Mapping Build Error"
			)
			raise
	...
	def get_cached_field_mapping(self, policy_type):
		...
		try:
			...
			frappe.log_error(
				f"Unexpected error while getting cached field mapping for {policy_type}: {str(e)}",
	...
	def build_dynamic_extraction_prompt(self, policy_type, extracted_text):
		...
		try:
			...
			frappe.log_error(
				f"Error building dynamic extraction prompt for {policy_type}: {str(e)}",
				"Dynamic Prompt Error",
			)
	...
	def _build_motor_extraction_prompt(self, extracted_text):
		...
		try:
			...
			for field in meta.fields:
				if (
					field.fieldname not in skip_fields
					...
					if any(keyword in field.fieldname.lower() for keyword in ["policy", "date"]):
						...
						for keyword in ["premium", "sum", "gst", "ncb", "amount"]
					):
						financial_fields.append(field_info)
					elif any(
						keyword in field.fieldname.lower()
						for keyword in ["vehicle", "make", "model", "engine", "chassis", "fuel", "cc", "rto"]
					):
						vehicle_fields.append(field_info)
					elif any(
						keyword in field.fieldname.lower()
						for keyword in ["customer", "payment", "bank", "branch"]
					):
			...
			for field in meta.fields:
				if (
					...
					if any(keyword in field.fieldname.lower() for keyword in ["policy", "date", "period"]):
						...
						keyword in field.fieldname.lower()
						for keyword in ["insured", "name", "birth", "relationship", "nominee"]
					):
						personal_fields.append(field_info)
					elif any(
						keyword in field.fieldname.lower() for keyword in ["premium", "sum", "gst", "amount"]
					):
						financial_fields.append(field_info)
					else:
						policy_fields.append(field_info)  # Default to policy fields
...
RESPOND WITH VALID FLAT JSON ONLY - NO EXPLANATIONS, NO MARKDOWN, NO CODE BLOCKS."""
			...
			frappe.log_error(
				f"Error building health extraction prompt: {str(e)}", "Health Prompt Build Error"
			)
