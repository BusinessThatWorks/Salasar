...
class PolicyDocument(Document):
		...
		if (
			...
			try:
				...
				if old_ctrl:
					...
					frappe.logger().info(
						f"Auto-populated old_control_number: {old_ctrl} " f"via before_save for {self.name}"
					)
				else:
	...
	def validate(self):
		...
		if (
			self.extraction_policy_type
			and self.policy_type
			and self.extraction_policy_type != self.policy_type
		):
			frappe.throw(
				f"Invalid input: Cannot change policy type after extraction. Extracted fields were generated for '{self.extraction_policy_type}' policy type. Please reprocess the document if you need to change the policy type."
			)
	...
	def _populate_processor_info(self):
		...
		try:
			...
			if employee:
				...
				frappe.logger().info(
					f"Auto-populated processor info and checklist fields for Policy Document: {employee}"
				)
			else:
	...
	def _populate_insurance_company_branch(self):
		...
		- Private company: only company_name extracted → search by name
		- Public company: both company_name + insurer_branch_code extracted → search by both
		"""
		try:
			extracted = frappe.parse_json(self.extracted_fields)
	...
	def process_policy(self, background=True):
		...
		if background:
			...
			try:
				...
				frappe.log_error(
					f"Unexpected error while processing policy document {self.name}: {str(e)}",
	...
	def _update_document_status(self, result, processing_time):
		...
		if result.get("success"):
			...
			frappe.log_error(
				f"Claude Vision processing failed for {self.name}: {self.error_message}",
				frappe.get_traceback(),
			)
	...
	def get_extracted_fields_display(self):
		...
		try:
			...
			frappe.log_error(
				f"Error parsing extracted fields for {self.name}: {str(e)}", "Policy OCR JSON Error"
			)
			return {}
	...
	def _fetch_old_control_number(self, engine_no):
		...
		if results:
			frappe.logger().info(
				f"Found old_control_number: {results[0].get('saiba_control_number')} for engine_no: {engine_no}"
			)
	...
	def reset_processing_status(self):
		...
		try:
			if self.status == "Processing":
				...
				frappe.msgprint(
					"Processing status has been reset. You can now retry processing.",
					title="Status Reset",
					indicator="blue",
				)
				...
				frappe.throw(
					f"Cannot reset status. Current status is '{self.status}', only 'Processing' status can be reset."
				)

		except Exception as e:
			# Use Frappe error logging
			frappe.log_error(
				f"Failed to reset processing status for {self.name}: {str(e)}", "Status Reset Error"
			)
	...
	def check_processing_timeout(self):
		...
		if self.status == "Processing" and self.modified:
			...
			from frappe.utils import now_datetime, time_diff
			...
			if minutes_elapsed > 10:
				...
				frappe.log_error(
					f"Document {self.name} processing timed out after {minutes_elapsed} minutes",
					"Processing Timeout",
				)
...
def process_policy_background(doc_name):
	...
	try:
		...
		try:
			...
			frappe.log_error(
				f"Failed to update document status for {doc_name}: {str(save_error)}",
				"Policy OCR Status Update Error",
			)
