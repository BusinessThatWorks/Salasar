...
policy_reader.saiba.show_validation_modal = function (result, on_sync) {
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action: function () {
			if (!ready) {
				...
				frappe.confirm(
					__(
						"Some required fields are missing or invalid. SAIBA may reject this policy. Continue anyway?"
					),
					function () {
						dialog.hide();
						if (on_sync) on_sync();
	...
	if (!ready) {
		dialog.$wrapper.find(".btn-primary").removeClass("btn-primary").addClass("btn-warning");
	}
...
policy_reader.saiba.show_ai_progress = function (frm) {
	...
	frm.dashboard.set_headline(`AI Extraction (SAIBA): ${filled} / ${total}`);
};
